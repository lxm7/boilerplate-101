...
describe('utils', () => {
  ...
  describe('fetchCurrencies promise -', () => {
    it('handles successful and returns data', async () => {
      await expect(fetchCurrencies() as Promise<any>).resolves.toBeTruthy();
    });

    it('the fetch throw when a bad param is passed in', async () => {
      await expect(fetchCurrencies('some') as Promise<any>).rejects.toThrow(
