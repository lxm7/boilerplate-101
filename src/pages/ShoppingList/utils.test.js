import {roundToTwo, getTotal, findSelectedCurrency, fetchCurrencies} from './utils';
...
describe("utils", () => {
  describe('roundToTwo -', () => {
    it('operates on input as expected', () => {
      ...
      expect(shortDecimalNo).toEqual(1.10);
    });
  });
  ...
  describe('fetchCurrencies promise -', () => {
    ...
    it('the fetch throw when a bad param is passed in', async () => {
      await expect(fetchCurrencies("some")).rejects.toThrow("Request failed with status code 400");
    });
  });
});
